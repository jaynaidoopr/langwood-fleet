...
import { ArrowLeft, Fuel, Navigation, Wrench, LocateFixed } from 'lucide-react';
...
export default function VehicleProfilePage() {
  ...
  const { setSelectedVehicleId, setFocusMode } = useMapContext();
  const { showError, showSuccess } = useFlash();
    ...
    <div className="space-y-3 md:space-y-5">
      ...
      <PageHeader title={`${v.name} · ${v.model}`} description={`${v.plate} · ${v.group}`}
        actions={<><StatusBadge status={v.status} />
          ...
          <Button size="sm" onClick={handleViewOnMap}><LocateFixed className="h-4 w-4" /> {t('liveMap')}</Button>
        </>}
      />
      ...
      <Tabs defaultValue="overview">
          ...
          <div className="grid gap-2 lg:gap-4 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="flex flex-col gap-2">
                <Button variant="outline" className="justify-start" onClick={handleViewOnMap}><Navigation className="h-4 w-4" /> {t('liveMap')}</Button>
        ...
        <TabsContent value="trips">
          ...
          : vehicleTrips.length === 0 ? <Card><CardContent className="p-0"><div className="p-6"><EmptyState title={t('noTrips30d')} /></div></CardContent></Card>
          : <div className="space-y-3">
              {vehicleTrips.map((trip: any) => (
                <Card key={trip.id}>
                  <CardContent className="p-4">
                    {/* Start / End Time header row */}
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="text-xs font-semibold text-emerald-600">{t('startTime')}</div>
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-destructive">{t('endTime')}</div>
                      </div>
                    </div>
                    <div className="mt-1 grid grid-cols-2 gap-3 border-b border-border pb-3">
                      <div className="text-xs">
                        <div className="text-foreground">{formatDate(trip.startTime)}</div>
                        <div className="text-muted-foreground">{t('reportColStartOdometer')}: {trip.startOdometer != null ? `${trip.startOdometer.toLocaleString()} km` : '—'}</div>
                      </div>
                      <div className="text-xs">
                        <div className="text-foreground">{formatDate(trip.endTime)}</div>
                        <div className="text-muted-foreground">{t('reportColEndOdometer')}: {trip.endOdometer != null ? `${trip.endOdometer.toLocaleString()} km` : '—'}</div>
                      </div>
                    </div>

                    {/* Stats grid */}
                    <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                      <div><span className="text-muted-foreground">{t('distance')}: </span><span className="font-medium">{formatDistance(trip.distance)}</span></div>
                      <div><span className="text-muted-foreground">{t('averageSpeed')}: </span><span className="font-medium">{trip.avgSpeed} {t('unitKmh')}</span></div>
                      <div><span className="text-muted-foreground">{t('duration')}: </span><span className="font-medium">{formatDuration(trip.duration)}</span></div>
                      <div><span className="text-muted-foreground">{t('maximumSpeed')}: </span><span className="font-medium">{trip.maxSpeed} {t('unitKmh')}</span></div>
                      {trip.fuelUsed != null && (
                        <div className="col-span-2"><span className="text-muted-foreground">{t('spentFuel')}: </span><span className="font-medium">{trip.fuelUsed.toFixed(2)} L</span></div>
                      )}
                    </div>

                    {/* Addresses */}
                    <div className="mt-3 space-y-0.5 text-xs text-muted-foreground">
                      <div><span className="font-medium text-foreground">{t('reportColStartAddress')}: </span>{trip.startAddress || '—'}</div>
                      <div><span className="font-medium text-foreground">{t('reportColEndAddress')}: </span>{trip.endAddress || '—'}</div>
                    </div>
                  </CardContent>
                </Card>
              ))}
