...
// --- Basic Auth fallback -------------------------------------------------
// Cross-origin session cookies require SameSite=None; Secure, which only
// works over HTTPS. When talking to a plain-HTTP server (e.g. testing
// directly against a VPS IP with no TLS), the cookie can never persist.
// Traccar also accepts standard HTTP Basic Auth on every request, which
// has no cookie/HTTPS dependency at all — so we keep credentials in memory
// (never persisted to disk) and attach them as a fallback whenever present.
