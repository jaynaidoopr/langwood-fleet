...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string, code?: string) => {
    ...
    // Cookie-based sessions don't survive cross-origin over plain HTTP.
    // Basic Auth has no such dependency, so keep it as a fallback for
    // every subsequent request this session.
