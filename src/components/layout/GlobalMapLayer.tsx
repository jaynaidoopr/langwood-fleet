...
export default function GlobalMapLayer({ showControls = false }: GlobalMapLayerProps) {
  ...
  const { selectedVehicleId, setSelectedVehicleId, focusMode, setFocusMode, mapHandleRef } = useMapContext();
  ...
  const mapVehicles = useMemo(() => {
    if (focusMode && selectedVehicleId != null) {
      return vehicles.filter((v) => v.id === selectedVehicleId);
    }
    return vehicles;
  }, [vehicles, focusMode, selectedVehicleId]);
  ...
  useEffect(() => {
    if (prevFocusModeRef.current && !focusMode) {
      // Wait a tick so FleetMapLibre has re-rendered with the full vehicle list
      // before fitting bounds — otherwise it'd fit to the old filtered set.
      requestAnimationFrame(() => mapHandleRef.current?.fitAllVehicles());
    }
    prevFocusModeRef.current = focusMode;
  }, [focusMode, mapHandleRef]);
  ...
  return (
    <div className="absolute inset-0">
      <FleetMapLibre
        ...
        vehicles={showControls ? mapVehicles : []}
      ...
      {showControls && (
        <div className="absolute left-2 top-2 z-20 flex flex-col gap-1 sm:left-4 sm:top-14">
          <div className="flex flex-wrap items-center gap-0.5 rounded-lg border border-border bg-card/95 p-0.5 shadow-md backdrop-blur">
            ...
            <button type="button" onClick={() => { if (focusMode) { setFocusMode(false); } else { mapHandleRef.current?.fitAllVehicles(); } }}
